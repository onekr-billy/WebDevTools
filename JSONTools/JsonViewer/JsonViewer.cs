...
namespace EPocalipse.Json.Viewer
{
    public partial class JsonViewer : UserControl
    {
        ...
        //readonly Configuration _configManage = ConfigurationManager.OpenExeConfiguration(Application.StartupPath + "\\appdata.config");
        readonly Configuration _configManage = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
        /// <summary>
        /// 初始化 控件
        /// </summary>
        private void CurrentControlInit()
        {
            cboxContentType.SelectedIndex = 0;
            cboxAccept.SelectedIndex = 0;
            cboxMethod.SelectedIndex = 0;

            var historyUrls = (_configManage.AppSettings.Settings["HistoryUrl"].Value ?? "").Split(new[] { ',', '|' }, StringSplitOptions.RemoveEmptyEntries);
            if (historyUrls.Length > 0)
            {
                foreach (var historyUrl in historyUrls)
                {
                    cboxHttpUrl.Items.Add(historyUrl);
                }

                if (cboxHttpUrl.Items.Count > 0)
                    cboxHttpUrl.SelectedIndex = 0;
            }
        }

        private delegate void SetJsonTxt(string txt);
